...
def attach_discount_to_realm(
    realm: Realm, discount: Decimal, acting_user: Optional[UserProfile] = None
) -> None:
    customer = get_customer_by_realm(realm)
...
def update_sponsorship_status(
    realm: Realm, sponsorship_pending: bool, acting_user: Optional[UserProfile] = None
) -> None:
...
def approve_sponsorship(realm: Realm, acting_user: Optional[UserProfile] = None) -> None:
...
def update_billing_method_of_current_plan(
    realm: Realm, charge_automatically: bool, acting_user: Optional[UserProfile] = None
) -> None:
    plan = get_current_plan_by_realm(realm)
    if plan is not None:
